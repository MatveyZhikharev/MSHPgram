...
export default {
  ...
  methods: {
    ...
    async getMessages() {
      let response = await axios.get('/messages/all', {
        params: {
          chat: this.chat
        }
      })
      for (let i = 0; i < response.data.length; i++) {
        response.data[i].user = await this.getUser(response.data[i].user)
      }
...
<template>
  <div>
    <div class="p-5 messages">
      <button
          v-if="chat"
          @click="goEdit"
          class="btn btn-secondary position-fixed">Редактировать чат
      </button>
      <div
        ...
        <div class="position-relative w-100 my-3">
          <span
              v-if="index === 0 || messages[index-1].user._id !== item.user._id"
              class="position-absolute"
              :class="{
          'right': item.user._id === user_id
        }">{{ item.user.name }}</span>
        </div>
        <div
            :class="{
        'pt-2': index === 0 || messages[index-1].user._id !== item.user._id
        }">
          <img
              v-if="item.user.username !== username && (index === 0 || messages[index-1].user._id !== item.user._id)"
              class="top-3"
              :src="item.user.avatar"
              alt="">
          <div
              class="message-box mx-5 position-relative"
              :class="{
        'mt-4': item.user.username === username && (index === 0 || messages[index-1].user._id !== item.user._id)
        }">
            <div
                :class="{
              'float-start': item.user._id !== user_id,
              'float-end': item.user._id === user_id
            }">
              <div class="message p-1">
                <span v-html="htmlize(item.text)"></span>
              </div>
            </div>
            <img
                v-if="item.user.username === username && (index === 0 || messages[index-1].user._id !== item.user._id)"
                class="position-absolute right-img"
                :src="item.user.avatar"
                alt="">
          </div>
        </div>
    ...
    <div class="message-row mx-5 position-relative">
      <textarea
          class="message-input p-4"
          placeholder="Введите текст"
          v-model="message"></textarea>
      <span
          @click="sendMessage"
          class="position-absolute send-button">
        <svg width="60px" height="60px" viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
